...
def _teardown_profiling_state(runner: "GPUModelRunner") -> None:
    ...
    # Invalidate the align-mode Mamba group metadata cached from the profiling
    # KVCacheConfig: ``_get_mamba_group_info`` memoizes group ids and the spec
    # from whichever config it first sees, and the real (e.g. PP-projected)
    # config may place Mamba layers into a different group layout, so it must
    # be re-derived from the real config.
