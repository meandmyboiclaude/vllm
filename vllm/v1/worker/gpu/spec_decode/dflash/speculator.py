...
class DFlashSpeculator(DraftModelSpeculator):
    ...
    def capture(self) -> None:
        ...
        # Padded sample rows must not scatter into a live request during
        # capture: the kernel's invalid-row sentinel is -1, so zero_() would
        # bake every padded row onto request 0 and corrupt its draft at every
        # FULL-graph replay.  [upstream vllm#47808, hunk 1 of 2]
...
def _prepare_dflash_inputs_kernel(
    ...
    if block_idx == 0:
        ...
        # tokens this step.
        # [upstream vllm#47808, hunk 2 of 2] clamp to max_model_len: an
        # unclamped absolute seq_len runs the draft attention past the end
        # of the block table near the context limit.
        tl.store(
