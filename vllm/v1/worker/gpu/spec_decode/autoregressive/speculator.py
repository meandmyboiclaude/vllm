...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    ...
    def propose(
        ...
        # no DP rank requires draft tokens this step (on the dp_sync reuse
        # path there is no collective to agree it, so it stays False there).
        no_draft_mask = input_batch.no_draft_mask_np
        want_skip_drafts = bool(no_draft_mask is not None and no_draft_mask.all())

        prefill_batch_desc, prefill_batch_sync, skip_all_drafts = (
        ...
        decode_batch_desc, decode_batch_sync, _ = dispatch_cg_and_sync_dp(
