...
def rejection_sample(
    ...
    if not sampling_metadata.all_random:
        ...
        if use_relaxed:
            ...
            topk_values, topk_indices = torch.topk(target_logits, k=relax_top_k, dim=-1)
            ...
            rejection_greedy_sample_kernel[(batch_size,)](
                ...
                is_greedy,
                max_spec_len,
                start_token_id if relaxed_thinking else NO_THINK_BOUNDARY_TOKEN_ID,
                end_token_id if relaxed_thinking else NO_THINK_BOUNDARY_TOKEN_ID,
                uniform_probs,
...
def rejection_greedy_sample_kernel(
    ...
    for pos in range(num_draft_tokens):
        if not rejected:
            ...
            is_think_start_token = draft_token_id == think_start_token_id
            is_think_end_token = draft_token_id == think_end_token_id
            if accepted and (is_think_start_token or is_think_end_token):
                rejected = True
...
def relaxed_thinking_sample_kernel(
    ...
    end_idx = tl.load(cu_num_draft_tokens_ptr + req_idx)
    num_draft_tokens = end_idx - start_idx
    thinking = tl.load(thinking_states_ptr + req_idx) != 0
            ...
            if thinking:
                ...
                for i in range(K):
                    if not accepted:
                        ...
                        if (candidate_logit >= logit_floor) and (
                            draft_token_id == candidate_id
                        ):
    ...
    for pos in range(num_draft_tokens):
        if not rejected:
            ...
            if accepted:
                ...
                fallback_id = 0
                # Reconstruct target probabilities on the fly from logits using
                # the online-softmax normalization state, avoiding a full
                # [num_tokens, vocab_size] target_probs materialization.
                recovery_target_max = tl.load(target_max_ptr + token_idx)
                recovery_target_inv_sum = tl.load(target_inv_sum_ptr + token_idx)
                for v in range(0, vocab_size, BLOCK_SIZE):
                    vocab_offset = v + tl.arange(0, BLOCK_SIZE)
                    vocab_mask = vocab_offset < vocab_size
                    ...
                    target_prob_v = (
                        tl.exp2(
                            (target_logit_v - recovery_target_max) * 1.4426950408889634
                        )
                        * recovery_target_inv_sum
                    )
