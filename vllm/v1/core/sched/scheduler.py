...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        if speculative_config is not None:
            ...
            # num_lookahead_tokens comes from the VllmConfig property (already
            # per-method: DFlash nst+1, eagle/draft/DSpark nst) — do not
            # re-derive it here; scheduler and warmup must not drift apart.
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # Structured output: grammar masks cover the scheduled draft
            # positions, but an accepted block can still carry an
            # unconstrained token past a reasoning transition or past grammar
