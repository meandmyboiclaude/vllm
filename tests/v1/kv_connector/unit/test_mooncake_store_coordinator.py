...
def test_lookup_with_eagle_hybrid_full_plus_mamba_no_overrun():
    ...
    ``MambaManager`` ignores ``drop_eagle_block`` (a Mamba block at
    position p IS the recurrent state after (p + 1) * block_size tokens;
    there is nothing to recompute), so granting the Mamba group the
    one-block eagle peek lets it match one block PAST the eagle-pruned
    full-attention hit, and adopting that length resumes the recurrent
    state ahead of the verified token prefix (#43559). The gate on
    ``supports_eagle_cache_peek`` plus the min() clamp on
    ``curr_hit_length`` pin the hit to the attention-verified 48.
    """
    groups = [
        KVCacheGroupSpec(["L0"], _full(16)),
