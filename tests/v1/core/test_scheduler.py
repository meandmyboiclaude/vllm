...
def test_long_prefill_threshold_does_not_truncate_spec_decode_padding():
    """#54028: the long_prefill_token_threshold clamp runs before the padding
    decision, so a padded first decode step keeps its full
    1 + num_spec_tokens span instead of being cut back to `threshold` while
    still carrying num_spec_tokens placeholder drafts."""
    num_spec = 3
    scheduler = create_scheduler(
        num_speculative_tokens=num_spec,
