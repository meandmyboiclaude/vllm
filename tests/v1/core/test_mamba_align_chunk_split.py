...
for every hash-cached mamba block. The simulation harness, block-tag state
oracle, poisoning test, and chunk-realignment test originate from #45477
(@kodek); the eagle and defer-(0)/progress cases are added on top.
"""
...
def _mamba_aligned_split(
    ...
    use_eagle: bool = True,
    num_uncached_common_prefix: int = 0,
) -> int:
    ...
    return Scheduler._mamba_block_aligned_split(
        ...
        req,
        num_new,
        num_new_local_computed,
        num_external_computed,
...
def test_split_guard_direct_cases():
    """Direct assertions on _mamba_block_aligned_split at the branch edges.

    With eagle, a sub-2-block prompt has no cacheable boundary
    (last_cache_position == 0): a budget-capped fragment must defer (0),
    while a chunk covering the whole prefill may end unaligned (its state
    slot is never hashed). A chunk crossing the last cacheable boundary of
    a longer prompt must stop exactly on that boundary.
    """
    short = create_requests(1, num_tokens=PROMPT_LEN, max_tokens=64)[0]
    assert _mamba_aligned_split(short, 364) == 0
    assert _mamba_aligned_split(short, PROMPT_LEN) == PROMPT_LEN

    long_len = 5 * MAMBA_BLOCK_SIZE + 7
    long_req = create_requests(1, num_tokens=long_len, max_tokens=64)[0]
    # last_cache_position = round_down(long_len, B) - B (eagle prune) == 4 * B
    assert (
        _mamba_aligned_split(long_req, 4 * MAMBA_BLOCK_SIZE + 600)
        == 4 * MAMBA_BLOCK_SIZE
    )


@pytest.mark.parametrize("use_eagle", [False, True])
def test_marconi_common_prefix_cap_keeps_alignment(use_eagle):
    """The Marconi admission cap must act on the chunk END, before alignment.

    Rounding the capped chunk *length* down to a block multiple (the
    previous form) reintroduces an unaligned non-final chunk end whenever
    the start offset is unaligned (external KV-connector tokens): start 368
    + round_down(2000) = 2368 -> length-rounded end 1968, mid block — the
    exact snapshot-poisoning geometry the alignment branches exist to
    prevent. Capping the end position first and aligning afterwards stops
    the chunk at 1600 instead.
    """
    prompt_len = 5 * MAMBA_BLOCK_SIZE + 7
    req = create_requests(1, num_tokens=prompt_len, max_tokens=64)[0]
    ...
    num_new = _mamba_aligned_split(
        req,
        prompt_len - 368,
        num_external_computed=368,
        use_eagle=use_eagle,
        num_uncached_common_prefix=2000,
    )
    # Capped end 368 + 2000 = 2368, aligned down to the 1600 boundary.
    assert num_new == MAMBA_BLOCK_SIZE - 368

    # Aligned start: identical to the historical length-rounding behavior.
    assert (
        _mamba_aligned_split(
            req,
            prompt_len,
            use_eagle=use_eagle,
            num_uncached_common_prefix=2000,
...
def test_deferred_fragment_progresses_with_block_budget():
    """Defer-0 is bounded, not a livelock.

    A non-final chunk whose leftover step budget cannot reach the next
    mamba boundary schedules 0 tokens and retries next step. Align-mode
    config validation (VllmConfig.validate_block_size) asserts both
    block_size <= max_num_batched_tokens and long_prefill_token_threshold
    in {0} or [block_size, inf), so every drained step offers at least
    one full block of budget and each defer-(0) is followed by progress.
    """
    prompt_len = 5 * MAMBA_BLOCK_SIZE + 7
    req = create_requests(1, num_tokens=prompt_len, max_tokens=64)[0]
    req.num_computed_tokens = MAMBA_BLOCK_SIZE

    # Leftover budget one short of the next boundary: defer.
    assert _mamba_aligned_split(req, MAMBA_BLOCK_SIZE - 1) == 0
    # A full-block budget always makes progress.
    assert _mamba_aligned_split(req, MAMBA_BLOCK_SIZE) == MAMBA_BLOCK_SIZE
